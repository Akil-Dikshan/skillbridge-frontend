...
const ReviewModal = ({ bookingId, mentorId, mentorName, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await createReview({ bookingId: parseInt(bookingId), mentorId: parseInt(mentorId), rating, feedback: comment });
      setDone(true);
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" style={sg}>
            <div className="px-6 pt-6 pb-5" style={{ background: 'linear-gradient(135deg, #1a1756 0%, #312e81 60%, #4338ca 100%)' }}>
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-xl font-bold text-white leading-tight">Rate your session</h2>
                  <p className="text-indigo-200 text-sm mt-0.5">with {mentorName || `Mentor #${mentorId}`}</p>
                </div>
                <button onClick={handleClose} className="p-1.5 rounded-lg text-indigo-300 hover:text-white hover:bg-white/10 transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            {done ? (
              <div className="px-6 py-10 text-center">
                ...
                <button onClick={handleClose} className="px-6 py-2.5 text-sm font-semibold text-white rounded-xl"
              ...
              <form onSubmit={handleSubmit} className="px-6 py-6 space-y-5">
                <div className="text-center">
                  <div className="flex items-center justify-center gap-2 mb-2" onMouseLeave={() => setHovered(0)}>
                    {[1,2,3,4,5].map((star) => (
                      <button key={star} type="button" onClick={() => setRating(star)} onMouseEnter={() => setHovered(star)}
                        className="focus:outline-none transition-transform hover:scale-110 active:scale-95">
                        <Star className="w-10 h-10 transition-colors"
                          style={{ fill: star <= activeRating ? '#F59E0B' : 'transparent', color: star <= activeRating ? '#F59E0B' : '#D1D5DB' }} />
                      </button>
                    ))}
                  </div>
                  <p className="text-sm font-semibold text-gray-500 h-5">{activeRating > 0 ? LABELS[activeRating] : 'Select a rating'}</p>
                </div>
                ...
                <div>
                  ...
                  <textarea value={comment} onChange={(e) => setComment(e.target.value)}
                    ...
                    rows={3} className="w-full px-4 py-3 text-sm text-gray-900 placeholder:text-gray-400 bg-gray-50 border border-gray-200 rounded-xl resize-none focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition-colors" />
                </div>
