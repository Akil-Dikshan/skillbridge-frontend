      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden flex hover:border-indigo-200 hover:shadow-lg hover:shadow-indigo-50/60 transition-all duration-200">
        <div className="w-44 flex-shrink-0 relative overflow-hidden bg-gray-100">
          {mentor.profilePictureUrl ? (
            <img src={mentor.profilePictureUrl} alt={name} className="w-full h-full object-cover" style={{ minHeight: '180px' }} />
          ) : (
            <div className="w-full flex items-center justify-center text-white font-bold text-3xl"
        ...
        <div className="flex-1 px-6 py-5 flex flex-col justify-between min-w-0">
          <div>
            <div className="flex items-start justify-between gap-3 mb-1">
              <div>
                <h3 style={sg} className="text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition-colors leading-tight">{name}</h3>
                {exp > 0 && <p className="text-sm text-indigo-500 font-medium mt-0.5">{exp}+ Years of Industry Experience</p>}
              </div>
            ...
            {skills.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                {skills.slice(0, 5).map(skill => (
                  <span key={skill} className="px-2.5 py-1 text-xs font-medium text-gray-600 bg-gray-100 rounded-full">{skill}</span>
                ))}
                {skills.length > 5 && (
                  <span className="px-2.5 py-1 text-xs font-medium text-gray-400 bg-gray-50 rounded-full">+{skills.length - 5}</span>
                )}
              </div>
            )}
          </div>
          <div className="flex items-center justify-between pt-3 border-t border-gray-50">
            <div>
              ...
              <p style={sg} className="text-lg font-bold text-gray-900">LKR {mentor.hourlyRate}<span className="text-sm font-normal text-gray-400">/hr</span></p>
            </div>
            <span className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-white rounded-xl group-hover:shadow-md group-hover:shadow-indigo-200 transition-all"
...
const SkeletonCard = () => (
  <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden flex animate-pulse" style={{ minHeight: '180px' }}>
    ...
    <div className="flex-1 px-6 py-5 space-y-3">
      ...
      <div className="flex gap-2">{[1,2,3].map(i => <div key={i} className="h-6 w-16 bg-gray-100 rounded-full" />)}</div>
    </div>
  </div>
);
...
const MentorListPage = () => {
  ...
  const skillCounts = useMemo(() => {
    ...
    allMentors.forEach(m => {
      if (!m.skills) return;
      m.skills.split(',').map(s => s.trim()).filter(Boolean).forEach(s => { map[s] = (map[s] || 0) + 1; });
    });
    ...
    <div className="space-y-0">
      <FilterSection title="Skills">
          ...
          {visibleSkills.map(([skill, count]) => (
            <label key={skill} className="flex items-center justify-between gap-2 cursor-pointer group">
              <div className="flex items-center gap-2">
                <input type="checkbox" checked={selectedSkills.has(skill)} onChange={() => toggleSkill(skill)}
        ...
        {skillCounts.length > 6 && (
          <button onClick={() => setShowMoreSkills(s => !s)}
        ...
        <div className="space-y-2">
          {[
            ...
            <label key={opt.value} className="flex items-center gap-2 cursor-pointer group">
              <input type="radio" name="exp" value={opt.value} checked={expFilter === opt.value}
      ...
      <FilterSection title="Price Range (LKR/hr)">
        <div className="flex items-center gap-2">
          <input type="number" value={minRate} onChange={e => setMinRate(e.target.value)} placeholder="Min" min="0"
          ...
          <input type="number" value={maxRate} onChange={e => setMaxRate(e.target.value)} placeholder="Max" min="0"
            className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white" />
        </div>
      </FilterSection>
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 style={sg} className="text-3xl sm:text-4xl font-bold text-gray-900 tracking-tight mb-1">Find your mentor</h1>
          <p className="text-gray-400 text-base mb-5">Connect with industry experts and accelerate your career.</p>
          ...
          <aside className="hidden lg:block w-64 flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 p-5 sticky top-20">
              <div className="flex items-center justify-between mb-5">
                ...
                {activeFilterCount > 0 && (
                  <span className="text-xs font-bold text-white bg-indigo-500 rounded-full px-2 py-0.5">{activeFilterCount}</span>
                )}
              </div>
              <FilterPanel />
            </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
              ...
              <button onClick={() => setFiltersOpen(true)}
                ...
                Filters
                {activeFilterCount > 0 && (
                  <span className="text-xs font-bold text-white bg-indigo-500 rounded-full w-5 h-5 flex items-center justify-center">{activeFilterCount}</span>
                )}
              </button>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20 bg-white rounded-2xl border border-red-100">
                ...
                <button onClick={() => fetchMentors()} className="text-xs text-indigo-600 hover:underline">Try again</button>
              </div>
              ...
              <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
                ...
                <button onClick={clearAll} className="text-sm font-semibold text-indigo-600 hover:underline">Clear all filters</button>
              </div>
            ) : (
              <div className="space-y-4">
                {filteredMentors.map(mentor => <MentorCard key={mentor.userId} mentor={mentor} />)}
              </div>
            )}
          </div>
        </div>
