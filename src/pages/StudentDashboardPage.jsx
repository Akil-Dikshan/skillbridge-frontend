      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-10">
        ...
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 style={sg} className="text-2xl font-bold text-gray-900 leading-tight">Hey, {firstName}</h1>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16"><p className="text-base text-red-400">{error}</p></div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  {displayList.length === 0 ? (
                    <div className="rounded-2xl border-2 border-dashed border-gray-100 py-16 text-center">
                      ...
                      <p className="text-sm text-gray-400 mb-6">
                        {activeTab === 'upcoming' ? 'Find a mentor and book your first session.' : 'Your session history will appear here.'}
                      </p>
                      {activeTab === 'upcoming' && (
                        <Link to="/mentors" className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white rounded-xl" style={{ background: '#4F46E5' }}>
                    ...
                    <div className="space-y-3">
                      {displayList.map((booking) => {
                        ...
                        return (
                          <div key={booking.id} className="flex items-stretch rounded-2xl border border-gray-100 overflow-hidden hover:border-gray-200 hover:shadow-sm transition-all">
                            ...
                            <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4">
                              <div>
                                <div className="flex items-center gap-2.5 flex-wrap">
                                  <Link to={`/mentors/${booking.mentorId}`} style={sg}
                                ...
                                <div className="flex items-center gap-4 mt-1.5 flex-wrap">
                                  ...
                                  {booking.status === 'CONFIRMED' && (
                                    <span className="flex items-center gap-1.5 text-sm text-emerald-500">
                                      <Video className="w-3.5 h-3.5" /> Link via email
                                    </span>
                                  )}
                                </div>
                                {booking.notes && (
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                {booking.status === 'COMPLETED' && !reviewedIds.has(booking.id) && (
                                  <ReviewModal bookingId={booking.id} mentorId={booking.mentorId}
                                    mentorName={`Mentor #${booking.mentorId}`}
                                    onSuccess={() => { setReviewedIds(prev => new Set([...prev, booking.id])); fetchData(); }} />
                                )}
                                {booking.status === 'COMPLETED' && reviewedIds.has(booking.id) && (
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                <div className="flex items-center justify-between px-6 pt-6 pb-4">
                  ...
                  <Link to="/mentors" className="text-sm font-semibold text-indigo-500 hover:text-indigo-600 flex items-center gap-0.5">
                    See all <ChevronRight className="w-4 h-4" />
                  </Link>
                </div>
                <div className="divide-y divide-gray-50">
                  {mentors.length === 0 ? (
                    <div className="px-6 pb-6 text-center"><p className="text-sm text-gray-400">No mentors found.</p></div>
                  ) : mentors.map((mentor) => {
                    ...
                    return (
                      <Link key={mentor.userId} to={`/mentors/${mentor.userId}`}
                        ...
                        <div className="flex-1 min-w-0">
                          <p style={sg} className="text-sm font-semibold text-gray-900 truncate group-hover:text-indigo-600 transition-colors">{name}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {skills[0] && <span className="text-xs text-gray-400 truncate">- {skills[0]}</span>}
                          </div>
                        </div>
                        <span style={sg} className="text-sm font-semibold text-indigo-600 flex-shrink-0">LKR {mentor.hourlyRate}</span>
                      </Link>
                    );
                  })}
                </div>
                <div className="px-6 py-4 border-t border-gray-50">
                  <Link to="/mentors" className="flex items-center justify-center gap-2 w-full py-3 text-sm font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-colors">
                ...
                <div className="space-y-5">
                  {[
                    ...
                    <div key={item.step} className="flex items-start gap-4">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 mt-0.5"
                        style={{ background: 'linear-gradient(135deg, #4F46E5, #7C3AED)' }}>
                        {item.step}
                      </div>
                      <div>
